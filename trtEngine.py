...
class BaseEngine(object):
    def __init__(self, engine_path):
        ...
        self.std = None
        self.n_classes = 80
        self.class_names = [ 'person', 'car', 'bike', 'motorcycle', 'airplane', 'bus', 'train', 'truck', 'boat', 'traffic light',
    ...
    def detect_video(self, video_path, conf=0.5, end2end=False):
        ...
        while True:
            ...
            # blob = np.repeat(blob[np.newaxis, ...], 8, axis=0)
            t1 = time.time()
    ...
    def detect_video_with_track(self, video_path, tracker, conf=0.5 ,end2end=False):
        cap = cv2.VideoCapture(video_path)
        fourcc = cv2.VideoWriter_fourcc(*'XVID')
        fps = int(round(cap.get(cv2.CAP_PROP_FPS)))
        width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
